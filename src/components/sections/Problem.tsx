    ...
    <section id="problem" className="section section--pale problem">
      <div className="container">
        ...
        <div className="stats-grid">
          ...
          <StatCounter end={80} display={(v) => `$50–${Math.max(50, Math.round(v))}`} label="средняя цена одной повязки QuikClot" />
          <StatCounter end={4} display={(v) => `${Math.max(1, Math.round(v))}+ часа`} label="среднее время ожидания скорой помощи в сельских районах" />
        </div>
      </div>
    </section>
  );
